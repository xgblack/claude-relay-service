...
const accountNameCacheService = require('../../services/accountNameCacheService.plus')
...
router.get('/usage-records', authenticateAdmin, async (req, res) => {
  try {
    ...
    for (const id of keyIds) {
      try {
        const info = await redis.getApiKey(id)
        if (info && Object.keys(info).length > 0) {
          keyInfoMap.set(id, {
            id,
            name: info.name || info.label || info.displayName || id
          })
        }
      } catch (error) {
        logger.debug(`⚠️ Failed to load api key info for ${id}: ${error.message}`)
      }
    }
    ...
    return res.status(500).json({ success: false, error: 'Failed to get usage records' })
  }
})

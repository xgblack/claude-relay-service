...
const fetchRecords = async (page = pagination.currentPage) => {
  loading.value = true
  try {
    const response = await apiClient.get(`/admin/plus/accounts/${accountId.value}/usage-records`, {
  ...
  try {
    ...
    while (page <= totalPages && page <= maxPages) {
      const response = await apiClient.get(
        `/admin/plus/accounts/${accountId.value}/usage-records`,
        {
          params: { ...buildParams(page), pageSize: 100 }
        }
      )
